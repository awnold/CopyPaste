...
angular.module('koan.common').factory('api', function ($rootScope, $http, $window) {
  ...
  api.pastes = {
    getPaste: function (id) {
      return $http({method: 'GET', url: apiBase+ '/pastes/' + id, headers: headers});
    },
    createPaste: function (data) {
      return $http({method: 'POST', url: apiBase + '/pastes/', data: data, headers: headers});
    }
